...
import { MoreVert, Edit, Delete, Add, WarningAmber, Settings } from '@mui/icons-material'
...
function Factures() {
  ...
  const addAnomalie = (id) => {
    navigate(`/user/addanomalie/${id}`)
  }

  const controleFacture = (id) => {
    navigate(`/user/addcontrolefacture/${id}`)
  }

  const updateFacture = (id) => {
    navigate(`/user/editfacture/${id}`)
  }

  const deleteFacture = async (id) => {
    await axios.delete(`${API}/factures/remove/${id}`)
    .then((res) => {
      if(res.data.success) {
                          ...
                          .map((row) => (
                            ...
                            >
                              ...
                              <TableCell align="right">
                                ...
                                >
                                {/*         <MenuItem onClick={()=>addAnomalie(row._id)}>
                                    <ListItemIcon >
                                      <WarningAmber />
                                    </ListItemIcon>
                                    <ListItemText>Anomalie</ListItemText>
                                  </MenuItem>*/
                                }
                                  <MenuItem onClick={()=>controleFacture(row._id)}>
                                    <ListItemIcon>
                                      <Settings />
                                    </ListItemIcon>
                                    <ListItemText>Controle</ListItemText>
                                  </MenuItem>
                                  <MenuItem onClick={()=>updateFacture(row._id)}>
                                    <ListItemIcon >
                                      <Edit />
                                    </ListItemIcon>
                                    <ListItemText>Edit</ListItemText>
                                  </MenuItem>
                                  <MenuItem onClick={()=>deleteFacture(row._id)}>
                                    <ListItemIcon>
                                      <Delete />
                                    </ListItemIcon>
